...
1. **The plugin's auto-activation hook runs `node`.** There is no node on the
   ranch Mac — the office app's `index.html` validator runs on JavaScriptCore
   via `osascript` for that reason. The hook would fail every session start.
...
`CLAUDE.md`, so this file stays a clean re-copy. The office app repo carries
the same two files; keep them in step.
